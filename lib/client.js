...
window.__ModuleLoader__.load({ id: '@yeqingtang/dsh-task-watcher-plugin', factory: (require) => {
  ...
  const T = {
    zh: {
      ...
      hint2: '开关只控制进程启停；停用/卸载本插件不会杀死正在运行的监视器。',
      loadFail: '状态获取失败',
    },
    en: {
      title: 'Task Watcher',
      ...
      hint2: 'The switch only controls the process; disabling/uninstalling this plugin never kills a running watcher.',
  ...
  function TaskWatcherPanel() {
    ...
    return h('div', { className: 'dtw-panel' },
      h('div', { className: 'dtw-row dtw-row-main' },
        ...
        h('button', { type: 'button', className: 'dtw-refresh', onClick: load, title: t('refresh') },
      ...
      status ? h('div', { className: 'dtw-meta' },
        running ? h('span', null, t('pid') + ' ' + status.pids.join(', ') + ' · ') : null,
        status.version ? h('span', null, t('version') + ' ' + status.version + ' · ') : null,
        h('span', { className: 'dtw-dir' }, status.dataDir),
      ) : null,
  ...
  const DTW_CSS = `
.dtw-panel { padding: 16px 4px; max-width: 720px; font-size: 13px; }
.dtw-row-main { display: flex; align-items: center; gap: 10px; }
.dtw-state { font-weight: 600; }
.dtw-dot { width: 9px; height: 9px; border-radius: 50%; flex: none; }
...
.dtw-switch { position: relative; width: 40px; height: 22px; border-radius: 11px; border: 1px solid rgba(128,128,128,.45); background: rgba(128,128,128,.25); cursor: pointer; padding: 0; margin-left: auto; transition: background .18s ease; }
.dtw-switch-on { background: #4d6bfe; border-color: #4d6bfe; }
.dtw-knob { position: absolute; top: 2px; left: 2px; width: 16px; height: 16px; border-radius: 50%; background: #fff; transition: left .18s ease; box-shadow: 0 1px 3px rgba(0,0,0,.35); }
.dtw-switch-on .dtw-knob { left: 20px; }
.dtw-switch:disabled { opacity: .55; cursor: default; }
.dtw-refresh { background: none; border: none; cursor: pointer; font-size: 15px; color: inherit; opacity: .6; padding: 2px 6px; }
.dtw-refresh:hover { opacity: 1; }
.dtw-meta { margin-top: 10px; opacity: .75; font-size: 12px; word-break: break-all; }
.dtw-error { margin-top: 8px; color: #ef4444; font-size: 12px; }
