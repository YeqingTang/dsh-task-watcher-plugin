...
const GUIDANCE =
  "本机已安装 dsh-task-watcher 插件（DSH 任务监视器外壳）：它把独立的 Windows 托盘监视程序 DshTaskWatcher 部署到 %LOCALAPPDATA%\\DshTaskWatcher 并拉起（CREATE_NO_WINDOW 无终端窗口直起、脱离宿主进程常驻）。托盘四态图标：灰=DSH 未连接 / 绿=空闲 / 橙=任务运行中 / 红=有待处理；单击托盘看监控窗。管理路由（仅 loopback POST）：/api/dsh-task-watcher/status|start|stop；Web GUI 设置 → 插件 → 任务监视器 有启停开关（只控进程，不改其独立生命周期）。用户提到「任务监视器/托盘监视器/DshTaskWatcher」时即指本插件，请据此协作。";
